...
export const AdminLayout = () => {
  ...
  // Security checks: user must be authenticated AND have role admin or seller
  if (!isAuthenticated) {
  ...
  if (user?.role !== 'admin' && user?.role !== 'seller') {
    return (
      <div className="container" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '400px', gap: '16px' }}>
        ...
        <h3>Access Denied</h3>
        <p className="text-secondary">You do not have administrative or seller permissions to access this area.</p>
